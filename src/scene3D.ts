...
export class Scene3D {
  ...
  private setupEnvironment(): void {
    ...
    this.dustPoints = new THREE.Points(dustGeometry, dustMaterial);
    this.scene.add(this.dustPoints);

    // Nebula background effect (simple mesh with gradient)
    const sphereGeo = new THREE.SphereGeometry(45, 32, 32);
  ...
  render(): void {
    ...
    // Animate dust
    if (this.dustPoints) {
      this.dustPoints.rotation.y += deltaTime * 0.02;
      this.dustPoints.rotation.x += deltaTime * 0.01;
    }
