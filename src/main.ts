...
class AirCanvas {
  ...
  private setupButtonListeners(): void {
    ...
    if (localStorage.getItem('air-canvas-tutorial-seen')) {
      tutorialOverlay?.classList.add('hidden');
    }

    closeTutorialBtn?.addEventListener('click', () => {
      tutorialOverlay?.classList.add('hidden');
      localStorage.setItem('air-canvas-tutorial-seen', 'true');
      this.audioManager.resume();
    });
  }
  ...
  private updatePreviewCanvasSize(): void {
    ...
    const cameraPreview = document.getElementById('camera-preview'); // Define cameraPreview here
    if (cameraPreview) {
  ...
  private handleDraw(position: { x: number; y: number }): void {
    ...
    // Play drawing sound based on vertical position
    const freq = 200 + (1 - position.y / window.innerHeight) * 600;
    this.audioManager.playDraw(freq);
  }
  ...
  private takeSnapshot(): void {
    ...
    tempCtx.drawImage(sceneCanvas, 0, 0);
    // Draw foreground (2D strokes)
    tempCtx.drawImage(drawCanvas, 0, 0);

    // Download
    const link = document.createElement('a');
    link.download = `air-canvas-snapshot-${Date.now()}.png`;
    link.href = tempCanvas.toDataURL('image/png');
    link.click();

    this.showStatus('Snapshot saved!', 2000);
  }
