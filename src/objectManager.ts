...
export class ObjectManager {
  ...
  constructor(scene: Scene3D, canvasWidth: number, canvasHeight: number, audioManager: AudioManager) {
    this.scene = scene;
  ...
  private async animateInflation(obj: BalloonObject): Promise<void> {
    return new Promise((resolve) => {
      ...
      gsap.delayedCall(0.3, () => {
        ...
        // Emit inflation particles
        this.scene.getParticleSystem().emit({
          count: 20,
          color: obj.color,
        ...
        this.audioManager.playInflate(obj.position.x, obj.position.y, obj.position.z);
      });
    });
  }
  ...
  update(deltaTime: number, elapsedTime: number): void {
    for (const obj of this.objects) {
      ...
      // Bobbing motion with slight gravity effect
      const gravity = 0.5;

      // Update vertical target position with gravity
      obj.targetPosition.y -= gravity * deltaTime;

      // Prevent falling below floor
      if (obj.targetPosition.y < -3) {
        obj.targetPosition.y = -3;
      }
          ...
          onComplete: () => {
            gsap.to(obj.mesh.scale, {
              ...
              onComplete: () => {
                ...
                // Play pop sound
                this.audioManager.playPop(obj.mesh.position.x, obj.mesh.position.y, obj.mesh.position.z);
